...
import { getUniverseCharacters, getUniverse, clipUrl } from "@/lib/api";
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {characters.map((character) => (
              <Link
                key={character.id}
                href={`/app/universe/${universeId}/characters/${character.id}`}
                className="bg-slate-900 rounded-xl border border-slate-800 transition-all hover:border-slate-700 flex overflow-hidden h-48"
              >
                {/* Character image */}
                {character.character_image_url ? (
                  <div className="shrink-0 w-32">
                    <img
                      src={clipUrl(character.character_image_url)}
                      alt={character.name}
                      className="w-full h-full object-cover"
                    />
                  </div>
                ) : (
                  <div className="shrink-0 w-32 bg-slate-800/50 flex items-center justify-center">
