...
function InlineTextarea({
  ...
  return (
    <p
      ...
      title="Click to edit"
    >
      {value || <span className="text-slate-500 italic">{placeholder || "Click to edit"}</span>}
      <Pencil className="w-3 h-3 text-slate-600 group-hover:text-slate-400 inline ml-1.5 -mt-0.5" />
    </p>
  );
}
...
export default function StudioCharacterProfilePage({
  ...
  const [activeTab, setActiveTab] = useState<"quotes" | "knowledge" | "voice" | "references">("quotes");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getCharacterReferenceImages(id).catch(() => ({ urls: [] as string[] })),
    ])
      .then(([char, items, charData, uni, vibeData, docs, voiceData, models, refData]) => {
        if (cancelled) return;
        setCharacter(char);
  ...
  const handleUploadRefImage = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        const data = await uploadCharacterReferenceImage(id, file);
        setRefImages(data.urls);
      } catch (err) {
  ...
  const handleDeleteRefImage = useCallback(
    async (url: string) => {
      try {
        const data = await deleteCharacterReferenceImage(id, url);
        setRefImages(data.urls);
      } catch (err) {
    ...
    <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <Link href={`/app/universe/${universeId}/projects/generate?character=${character.id}`}>
              <Button>
                <Sparkles className="w-4 h-4 mr-2" />
                Create Video
              </Button>
      ...
      <div className="border-b border-slate-800">
        <div className="flex gap-0">
          ...
          >
            <ImageIcon className="w-4 h-4 inline mr-2 -mt-0.5" />
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-semibold text-white">Reference Images</h2>
              <span className="text-sm text-slate-500">({refImages.length})</span>
            </div>
            <button
          ...
          <p className="text-sm text-slate-400">
            Upload up to 5 reference photos to anchor this character's visual identity. When generating portraits,
            the system preserves the likeness from these references while applying your selected video style.
            One reference uses standard generation; two or more uses enhanced multi-reference mode for stronger identity lock.
          </p>
          ...
          {refImages.length > 0 ? (
              ...
              {refImages.map((url) => (
                <div key={url} className="relative group">
                  <img
                    src={clipUrl(url)}
                    alt="Reference"
                    className="w-full aspect-[3/4] object-cover rounded-xl border border-slate-700 shadow-lg"
                  />
                  <button
                    onClick={() => handleDeleteRefImage(url)}
                    className="absolute top-1.5 right-1.5 w-6 h-6 bg-black/70 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600/80"
                  >
            ...
            <div
              onClick={() => refFileInput.current?.click()}
              className="py-16 rounded-xl border border-dashed border-slate-700 bg-slate-900/30 flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-slate-600 transition-colors"
            >
              <Upload className="w-8 h-8 text-slate-600" />
              <p className="text-sm text-slate-500">Click to upload a reference photo</p>
              <p className="text-xs text-slate-600">Up to 5 images, PNG/JPG/WebP, 10 MB each</p>
            </div>
          )}
        </div>
      )}
