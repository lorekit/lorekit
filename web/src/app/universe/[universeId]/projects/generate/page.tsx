...
function StepFormatAndCreate({
  ...
  const vibeKey = universe?.video_vibe_preset ?? "mobile_game";
  const vibeLabel = vibePresets[vibeKey]?.name ?? vibeKey;
          ...
          <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/50 p-6 text-center">
            {uploadedAudio ? (
              <div className="space-y-2">
                ...
                <audio
                  controls
                  src={`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8001"}/${uploadedAudio.file_path.replace(/^\.\//, '')}`}
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-white font-medium">Using: {vibeLabel}</p>
              <p className="text-xs text-slate-400">Video style is set per-universe</p>
            </div>
          </div>
          <a
...
export default function GenerateWizardPage({
    ...
    Promise.all([
      ...
      .then(async ([itemData, charData, voiceData]) => {
        ...
        const hasVibeImage = charData.images.some(
          (img: CharacterImage) => img.theme === vibeKey && img.url
        );
        if (!hasVibeImage && selectedCharacter.character_description) {
          try {
            await generateCharacterForCharacter(selectedCharacter.id, { theme: vibeKey });
  ...
  const handleGenerateStory = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/universe/${universeId}/projects/${result.project_id}`);
    } catch (err) {
      setGenerateError(
  ...
  }, [selectedCharacter, selectedItem, selectedArc, universe, router, universeId]);
