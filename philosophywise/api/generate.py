...
from philosophywise.models import Quote, StoryBreakdown
...
async def generate_story_endpoint(body: StoryRequest) -> dict:
    ...
    """
    from philosophywise.quotes.database import get_philosopher, select_quote_pair
    from philosophywise.story.generator import generate_story
    ...
    try:
        ...
        # Select or fetch quotes
        if body.hook_quote_id and body.truth_quote_id:
            # Legacy: explicit hook + truth IDs
            cursor = await conn.execute(
                "SELECT * FROM quotes WHERE id = ?", (body.hook_quote_id,)
            )
            hook_row = await cursor.fetchone()
            cursor = await conn.execute(
                "SELECT * FROM quotes WHERE id = ?", (body.truth_quote_id,)
            )
            truth_row = await cursor.fetchone()
            if not hook_row or not truth_row:
                raise HTTPException(status_code=404, detail="Quote not found")
            hook_quote = _row_to_quote(dict(hook_row))
            truth_quote = _row_to_quote(dict(truth_row))
        elif body.quote_ids and len(body.quote_ids) > 0:
            # Flexible: user picked quotes, backend assigns roles
            ...
            hook_quote, truth_quote = await select_quote_pair(conn, body.philosopher_id)
    finally:
        await conn.close()
...
async def _raw_concat(
    ...
    """
    from philosophywise.models import Scene as _Scene
